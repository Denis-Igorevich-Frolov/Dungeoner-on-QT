...
void MagicDefense::removeAllBonuses()
{
    ...
    recalculationChunks();
    /*Метод используется только в загрузке для предваритоельной очистки бонусов, он не посылает
     *сигнал об изменении вектора бонусов так как позже и так будет вызвано recalculateStats()*/
}
