...
void SecondarySkill::setValue(long newValue)
{
     ...
     *а HTML абзац с CSS стилем, где прописан межстрочный интервал в 70%.
     *Первый arg передаёт значение межстрочного интервала, а второй уже сам текст.
     *Также стоит помнить, что Qt больше не сможет корректно распознавать размер
     *лейбла, в который передан такой текст, а соответственно и центровать его.
     *К счастью погрешность легко определить. При изменении интервала сам размер
     *строки тоже становится меньше, считая даже первую строку. Из-за этой
     *уменьшенной первой строки, и получается некоторый сдвиг вниз размером в срезанные
