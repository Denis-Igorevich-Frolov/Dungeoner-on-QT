...
public:
    ...
    /*Добавление чанка в вектор бонусных чанков. В метод передаётся указатель на новый чанк, там его
    ...
    //Перегрузка идентичная по функционалу вышестоящему методу, только эта функция добавляет вектор значений
    void addBonusChunk(QVector<Chunk*> chunks);

    /*Удаление бонусного чанка. Метод имеет 4 перегрузки:
     *
     *В первой передаётся указатель на чанк, там итератор ищет первый с
     ...
     *
     *Во второй просто передаётся число, которое будет являться максимальным
     *значением, которое итератор будет искать среди максимальных значений
     *вектора бонусных чанков и удалять соответствующий указатель. Обход
     *так же идёт с конца.
     *
     *Ещё 2 перегрузки делают тоже самое что и вышеописанные, но с векторами
     *значений.
     *
...
private:
    ...
    //Вектор бонусных чанков, которые влезли в пределы и будут добавлены в конец прогрессбара
    ...
    //Перерасчёт общего вектора всех чанков
